...
export async function genqr(){
    ...
    for(let i =0;i<=b.length-1;i++){
       ...
       const  encrypted = CryptoJS.AES.encrypt(String(b[i]["ADM NO"]),"thisisthebestkeyforhashingit").toString()
        ...
        if(b[i]["Hash"]==null){
        await supabase.from('commerce').upsert({"ADM NO":b[i]["ADM NO"],"STUDENT NAME":b[i]["STUDENT NAME"],"CLASS":b[i]["CLASS"],"Hash":encrypted,})
...
export async function getFiles(){
    genqr()
const {data} =  await supabase.from('commerce').select('*')
console.log(data)
const b=data!=null?data:[]
for(let i =0;i<=b.length-1;i++){
    console.log(b[i])
