...
export async function check(adm:any,schl:any){
  const {data,error }= await supabase.from('Awards Log').select('*').eq('eno',adm).eq('schl',schl)
  if(error){
    alert(error)
 ...
 window.location.replace('/login')});*/
  const handleSubmit$ = $( async () => {
   ...
   // alert(c)
    ...
    const { error } = await supabase
    .from('Awards Log')
    .insert({ eno:route,name:name, time:dts, schl:schl,accompany:exercise })
    if(error){
      console.log(error)
        }
        else{
    ...
    alert('You have already marked for this student')
    state.qr=''
  }}
...
.select("*")
.eq(col,`${res}`)
if(data && data.length==1){state.data;state.route=data[0]["empno"];state.name=data[0]["name"];state.school=data[0]["schl"];state.email=data[0]["empno"];handleSubmit$()}else if(data && data.length>1){window.location.replace('/dupe/'+res)}else{console.log('no data')}
